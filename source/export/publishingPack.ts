...
function contactSheetSvg(campaign:Campaign){const cellW=360,cellH=450,gap=24,cols=Math.min(3,campaign.posts.length),rows=Math.ceil(Math.min(campaign.posts.length,9)/3),W=cellW*cols+gap*(cols+1),H=rows*cellH+gap*(rows+1);const cells=campaign.posts.slice(0,9).map((post,i)=>{const x=gap+(i%3)*(cellW+gap),y=gap+Math.floor(i/3)*(cellH+gap),nested=renderArtworkSvg(post,'portrait').replace('<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350">','<g transform="scale(.333333)">').replace('</svg>','</g>');return `<g transform="translate(${x} ${y})">${nested}</g>`}).join('');return{svg:`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="100%" height="100%" fill="#efefef"/>${cells}</svg>`,width:W,height:H}}
