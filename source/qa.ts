...
export function qaCampaign(campaign: Campaign): QaResult {
  ...
  if (campaign.mode === 'batch' && campaign.posts.length === 9) {
    const objectiveCounts = campaign.posts.reduce<Record<string, number>>((counts, post) => {
      counts[post.objective] = (counts[post.objective] ?? 0) + 1;
      return counts;
    }, {});
    if ((objectiveCounts.Awareness ?? 0) !== 3 || (objectiveCounts.Engagement ?? 0) !== 3 || (objectiveCounts.Conversion ?? 0) !== 3) {
      warnings.push(`Approved CTA mix is 3 Awareness, 3 Engagement and 3 Conversion posts. Current mix is ${objectiveCounts.Awareness ?? 0}/${objectiveCounts.Engagement ?? 0}/${objectiveCounts.Conversion ?? 0}.`);
    }
  ...
  for (let i = 1; i < campaign.posts.length; i++) {
    const previous = templates.find(template => template.id === campaign.posts[i - 1].template)?.family;
    const current = templates.find(template => template.id === campaign.posts[i].template)?.family;
    if (previous && previous === current) warnings.push(`Posts ${i} and ${i + 1} use the same template family consecutively.`);
  }
