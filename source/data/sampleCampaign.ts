...
const objectives = ['Awareness', 'Engagement', 'Conversion', 'Awareness', 'Engagement', 'Conversion', 'Awareness', 'Engagement', 'Conversion'] as const;
...
export const sampleCampaign: Campaign = {
  ...
  posts: headlines.map((headline, index) => ({
    id: `post-${index + 1}`,
    name: `Post ${index + 1}`,
    service: index === 2 || index === 5 ? 'Business Storage' : 'Self Storage',
    subtopic: index === 1 ? 'Unit sizes' : undefined,
    objective: objectives[index],
    template: templatePlan[index],
    eyebrow: index === 2 ? 'For local businesses' : 'Storage in Bridgwater',
    headline,
    answer: index === 5 ? 'Know what you are paying for and choose the space that suits you.' : 'Create useful space while life is busy, without having to part with everything at once.',
    ...
    caption: `${headline}. ${index === 5 ? 'Clear information makes choosing storage simpler.' : 'A practical option when you need more room for a while.'}`,
    suggestedDate: `2026-09-${String(2 + index * 2).padStart(2, '0')}`,
    suggestedTime: index % 2 ? '17:30' : '09:30',
    ctaType: objectives[index] === 'Conversion' ? 'Find out more' : objectives[index] === 'Engagement' ? 'Send us a message' : 'Awareness only',
    formats: ['portrait'],
